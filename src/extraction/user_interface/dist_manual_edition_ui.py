...
class MainWindow(Frame):
    ...
    def make_form(self, elements_data):
        ...
        tree.insert('', 'end', text='Inter-arrival distribution', values=(
        elements_data['dname'], elements_data['dparams']['mean'], elements_data['dparams']['arg1'],
        elements_data['dparams']['arg2']))
        return tree
    ...
    def close_window(self):
        for item in self.tree.get_children():
            ...
            self.new_elements.append(
                {'id': item, 'type': values[0], 'mean': str(values[1]), 'arg1': str(values[2]), 'arg2': str(values[3])})
        self.master.destroy()
...
class MyDialog:
    def __init__(self, parent, values, title):
        ...
        fields = [{'name': 'cb1', 'label': 'Probability distribution', 'type': 'Combobox', 'values': dist,
                   'current': values[0]},
    ...
    def ok(self, event=None):
        self.values = (
        self.entries['cb1'].get(), self.entries['in1'].get(), self.entries['in2'].get(), self.entries['in3'].get())
        self.top.destroy()
